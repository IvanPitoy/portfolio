...
@keyframes kiv-sub-flicker {
  ...
  .kiv-nav-cta:hover {
    ...
    box-shadow:0 0 28px rgba(200,169,110,0.4), 0 6px 18px rgba(0,0,0,0.4);
    color:#0A0A0A;
  }
  ...
  /* VIDEO EDITOR — Anton, glowing gold shimmer */
  .kiv-big-name {
  ...
  /* MOTION — closer to photo, moved up */
.kiv-big-sub-left {
  ...
  /* DESIGNER — closer to photo, moved up */
.kiv-big-sub-right {
  ...
  /* Making Every Frame Count — gold shimmer small */
.kiv-tagline {
  font-family:'Syne',sans-serif; font-weight:700;
...
.kiv-bottom-left { position:absolute; bottom:14rem; left:2.5rem; z-index:5; max-width:520px; }
  ...
  /* "hover each letter" hint — more visible */
  .kiv-hover-hint {
    position:absolute; top:"-1.8rem"; left:50%; transform:translateX(-50%);
    font-family:'Inter',sans-serif; font-size:0.65rem;
    letter-spacing:0.22em; text-transform:uppercase;
    color:rgba(200,169,110,0.8); white-space:nowrap;
    animation:kiv-hint-blink 2s ease-in-out infinite;
    pointer-events:none;
...
@media(max-width:700px){
  ...
  /* Big title */
  .kiv-big-name{font-size:clamp(2.2rem,9vw,4rem)!important;top:22%;}
  .kiv-big-sub-left{font-size:clamp(0.6rem,3.5vw,1.2rem)!important;left:3vw!important;top:42%;}
  .kiv-big-sub-right{font-size:clamp(0.6rem,3.5vw,1.2rem)!important;right:3vw!important;top:42%;}

  /* Wordmark */
  .kiv-wordmark{font-size:clamp(4rem,20vw,7rem)!important;}

  /* Bottom left content */
  .kiv-bottom-left{left:1rem;right:1rem;bottom:6rem;max-width:100%;}
  .kiv-tagline{font-size:0.85rem!important;}
  .kiv-subtext{font-size:0.78rem!important;}
  .kiv-long-text{display:none;}

  /* Hide floating badges on mobile — too cluttered */
...
useEffect(() => {
  ...
  // Hero is always visible so start immediately, pause when scrolled away
...
<span className="kiv-big-sub-right">DESIGNER</span>
        ...
        {/* Centered photo — in front of all text */}
...
{label:"Color Grading",      icon:"◈",  top:"74%", right:"27%", delay:"1s"},
  ...
  <div key={label} style={{
    ...
    borderRadius:"999px", padding:"0.35rem 0.8rem 0.35rem 0.5rem",
              whiteSpace:"nowrap",
              boxShadow:"0 4px 24px rgba(0,0,0,0.5), inset 0 1px 0 rgba(200,169,110,0.1), 0 0 0 1px rgba(200,169,110,0.05)",
...
<canvas ref={canvasRef} style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none",zIndex:3,willChange:"transform"}}/>
  ...
  {/* Long text — different font, more muted */}
